...
void trajectory_to_path(moveit_msgs::msg::RobotTrajectory trajectory, moveit::core::RobotStatePtr robot_state, const moveit::core::JointModelGroup* joint_model_group, rclcpp::Node::SharedPtr move_group_node)
{
...
int main(int argc, char ** argv)
{
	...
	auto path_pub = move_group_node->create_publisher<nav_msgs::msg::Path>("/ur3_path", 10);
	...
	// 调节比例阀，气压为0.6bar
	msg_pressure_adjust.data = 193;  // 示例压力值
	...
	target_pose.position.x = -0.375; // -0.3546
	...
	double fraction_qingkong = move_group.computeCartesianPath(waypoints_qingkong, eef_step, jump_threshold, trajectory_qingkong);
	...
	std::vector<geometry_msgs::msg::Pose> waypoints_qingkong1;

	target_pose.position.x = -0.375; // -0.3546
	target_pose.position.y = 0.050; // 0.079759
	target_pose.position.z = 0.275; // 0.274
	target_pose.orientation.x = 1.0;
	target_pose.orientation.y = 0.0;
	target_pose.orientation.z = 0.0;
	target_pose.orientation.w = 0.0;
	waypoints_qingkong1.push_back(target_pose);

	moveit_msgs::msg::RobotTrajectory trajectory_qingkong1;
	double fraction_qingkong1 = move_group.computeCartesianPath(waypoints_qingkong1, eef_step, jump_threshold, trajectory_qingkong1);
	
	moveit_msgs::msg::RobotTrajectory new_trajectory_qingkong1;
	new_trajectory_qingkong1 = scale_trajectory_speed(trajectory_qingkong1, 0.1);

	move_group.execute(new_trajectory_qingkong1);
	...
	// 添加从txt文件中提取的轨迹点 
	std::vector<geometry_msgs::msg::Pose> waypoints_1_1 = read_to_waypoints("/home/xht/moveit2_workspace/src/ur3_moveit/config/plan/test_gongyi.txt");
	...
	// 笛卡尔路径的速度规划
	// moveit_msgs::msg::RobotTrajectory trajectory_scaled_1 = scale_trajectory_speed(trajectory_1_1, 0.05);
	...
	// double rt_set_velocity = 0.00348;
	...
	double velocity_scaling_factor = 0.001;
	double acceleration_scaling_factor = 0.001;
	robot_trajectory::RobotTrajectory rt(move_group.getRobotModel(), move_group.getName());
    rt.setRobotTrajectoryMsg(*move_group.getCurrentState(), trajectory_1_1);
	moveit::planning_interface::MoveGroupInterface::Plan plan;

	// 对轨迹进行时间参数化
	time_param.computeTimeStamps(rt, velocity_scaling_factor, acceleration_scaling_factor);
	rt.getRobotTrajectoryMsg(trajectory_1_1);
	plan.trajectory_ = trajectory_1_1;


	trajectory_to_path(trajectory_1_1, robot_state, joint_model_group, move_group_node);
	...
	geometry_msgs::msg::Pose current_pose_without_header;

	current_pose_without_header = current_pose.pose;
	current_pose_without_header.position.z = 0.280; // 0.274
	waypoints_first.push_back(current_pose_without_header);
