...
int main(int argc, char ** argv)
{
	...
	auto move_group_node = rclcpp::Node::make_shared("new_ur3_moveit", node_options);
	...
	// 调节比例阀，气压为0.6bar
	msg_pressure_adjust.data = 200;  // 示例压力值
	...
	while(fraction < 1.0 && attempts < maxtries)
    {
        ...
        if(attempts % 10 == 0)
            RCLCPP_INFO(LOGGER, "Still trying after %d attempts...", attempts);
    }
    ...
    {   
        ...
        // bool retrysuccess = 
        iptp.computeTimeStamps(rt, 0.0027, 0.02);
        // iptp.computeTimeStamps(rt, move_group.max_velocity_scaling_factor_, move_group.max_acceleration_scaling_factor_);
